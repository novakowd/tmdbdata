...
> | 📝 | default of `auth_token = get_auth_token()`, will **only** work after the environment variable has been setup as mentioned [**above**](#authentication) |
> |-|:----------------------------------------------------|
...
> The functions `req_url_path_append()` and `req_url_query()` modified the url to `r base_tmdb_request() %>% req_url_path_append("search", "movie") %>% req_url_query(query = "Avengers") %>% purrr::pluck("url")`
...
The `$results` element is a nested list from the `json` response which then uses `tidyr::unnest_*()` to clean the data and get a result in a data frame (`tibble::tibble()`)
